...
//
// The turn must arrive at Bedrock in the order the client sent it.
func TestAnthropicIngressBedrockPreservesInterleavedThinkingOrder(t *testing.T) {
	...
	require.Equal(t, want, got, "interleaved turn must reach Bedrock in the order the client sent it")
}
